...
## DATOS ##
import os
os.chdir('C:/Users/gerar/Desktop/Master/Master/Machine learning - PART II/Data')
...
#vamos a ver esta base de datos con los datos ya estandarizados  y dividos las variables d eprediccion y las variables targert
...
### MODELOS ##
## creamops iuna lista de modelos con los modelos que queramos probar para este conjunto de datos 
models = []
...
for name, model in models:
    ...
    pipeline.fit(X_train,y_train) ##entrenamos el modelo
    prediction=pipeline.predict_proba(X_test)[:,1] ## me quedo con la probabilidad del si
    prediction=pd.DataFrame(prediction, columns=[name]) ##guardamos las predicciones en el dataframe
    
    prediciones.append(prediction) ##guardamos las predicciones
...
plt.show()
##
##coiibn esta grafica podemos ver si los algoritmos estan ahciendo lo mismo o no.
## como podemos ver el algoritmo LR y el LDA estan haciendo practicamente lo mismo mientras q el resto estan auhciendo cosas distintas.

##ahora vamso a ver un esambler muy basico (uno de ellos eseria coger el LR y el LDA y ahcer la porediccion media)
...
## si sopmos muy hipocondiracos yq ueremos que si algun modelo nos de que puede estar teniendo una enfermedad cardiaca nos quedamos con los valores maximo para que nos de que puede tener una alta probabilidad de enfermedad cardiaca, o al contrario si lo queremos al reves, obtendriamos los valores mas bajos
...
transformer = Binarizer(threshold=0.5).fit(Resultados_finales)  # fit does nothing. #ahora lo que hacemos es comparar aquellos cuya probabilidad sea mayor de 0.5 q tenga una enfermedad cardiaca (los clasifica con 0 y 1)
predicciones_ensemble=pd.DataFrame(transformer.transform(Resultados_finales))
print(predicciones_ensemble)
#por ejemplo segun vemos el primer registro, el algoritmo en la posicion 3 y en la posicion 7 da como que tiene probabnilidad de infarto mientras que el resto no, al igual con el 4, que dice que todos tienenprobabilidad de tener enfermedad cardiaca

##sihacemos esto podemos observar que alguno de estos modelas ensemble mejoran la precision de los modelos que hemos visto anteriormente
...
#########################queda arreglar esto  (el modelo 6 dice que es el que mejor quedama tanto en train como en test)
...
## medimos la matriz de confusion con el area por debajo de la curva y observar que algunos modelos son mejores y otros peores. RTenemos los modelos origniales y los esemble y los podemos compaarar
